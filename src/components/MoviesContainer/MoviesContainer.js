...
const MoviesContainer = (props) => {
  const apiEndpoints = {
    popular:
      '/movie/popular?api_key=466eefcef086aaa1375e8ecfebc6a345&language={}',
    upcoming:
      '/movie/upcoming?api_key=466eefcef086aaa1375e8ecfebc6a345&language={}',
    genre:
      '/discover/movie?api_key=466eefcef086aaa1375e8ecfebc6a345&language={}&with_genres=$',
    search:
      '/search/movie?api_key=466eefcef086aaa1375e8ecfebc6a345&language={}&include_adult=true&query=$',
  };
  ...
  useEffect(() => {
    if (currentPathname !== pathname || page !== search) {
      ...
      if (genre) {
        ...
        if (link) {
          url = apiEndpoints.genre
            .replace('{}', queryLanguage)
            .replace('$', link.id);
        }
      }
  ...
  if (requestState.movies) {
    content = <MovieList movies={requestState.movies} intl={props.intl} />;
  }
