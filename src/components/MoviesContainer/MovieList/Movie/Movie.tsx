...
const Movie: React.FC<Props> = ({ title, image, releaseYear, genre_ids, showDetails}) => {
    ...
    const genres = Object.values(genresLink).filter(g => genre_ids.includes(g.id as number)).map(g => g.text?.toLocaleString());
    ...
    const imageEl = image ? <img src={image} alt={`${title} Poster`}/> : <h4>{noPosterAvailable}</h4>
